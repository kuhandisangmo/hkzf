...
export default class RentAdd extends Component {
  ...
  // type : 当前操作的类型
  // index:移除图片的索引
  handleImgPick = (files, type, index) => {
    // 将选择的图片保存到tempSlides中
    this.setState({
      tempSlides: files
    })
  }
  ...
  addHouse = async () => {
    ...
    // 创建一个FormData 对象,来讲图片添加进来
    const form = new FormData()
    tempSlides.forEach(item => {
      form.append('file', item.file)
    })
    // 发送请求
    const resImg = await API.post(`/houses/image`, form)
